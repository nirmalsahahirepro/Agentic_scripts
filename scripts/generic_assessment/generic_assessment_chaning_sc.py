...
def generate_detailed_reports():
    ...
    try:
        ...
        excel_path = os.path.join(REPORT_DIR, f"Detailed_QA_Report_{report_tag}.xlsx")
        ...
        log.info(f"  ✅ [EXCEL] Detailed report with summary: {excel_path}")
    except Exception as e:
...
def main():
    ...
    log.info("═" * 60)
    log.info("Generic Assessment Agent - DETAILED QA MODE")
    ...
    try:
        ...
        # 2. RUN FIRST TEST STAGE
        if run_test_stage(driver, "STAGE 1 (Shortlisting)"):
            log_qa_step("Stage 1 Status", "PASS", "Completed first test")
        else:
            log_qa_step("Stage 1 Status", "FAIL", "Stage 1 failed")
            raise RuntimeError("Stage 1 failed")

        # 3. HANDLE CHAINING
        chain_status = step_handle_chaining(driver)
        log_qa_step("Eligibility Evaluation", "PASS", f"Result: {chain_status}")
        
        if chain_status == "PASS":
            log_qa_step("Chaining Transition", "PASS", "Initiating Stage 2")
            # 4. RUN SECOND TEST STAGE (7 MCQs)
            if run_test_stage(driver, "STAGE 2 (Final Test)", max_q=7):
                log_qa_step("Stage 2 Status", "PASS", "Completed stage 2")
            else:
                log_qa_step("Stage 2 Status", "FAIL", "Stage 2 execution failed")
        elif chain_status == "FAIL":
            log_qa_step("Chaining Transition", "N/A", "Candidate not eligible for next stage")
        else:
            log_qa_step("Chaining Transition", "WARNING", "Eligibility message not found")
